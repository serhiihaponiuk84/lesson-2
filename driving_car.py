...
class Car:
    ...
    def __init__(self, economy, color, model, mileage=configfile.DEFAULT_MILEAGE, fuel=configfile.DEFAULT_FUEL):
    ...
    def show_info(self):
        print(f"Mileage: {self.__mileage}, Fuel: {round(self.__fuel, configfile.SYMBOLS_AFTER_COMMA)}, Economy:"
...
def car_with_largest_remaining_fuel():
    if len(car_fleet):
        car_most_residual_fuel = max(car_fleet, key=lambda car: car.get_fuel_value, default=None)
        print("\nCar with the largest residual fuel:")
        car_most_residual_fuel.show_info()
        print(f"Fuel left for {round(car_most_residual_fuel.distance_left(), configfile.SYMBOLS_AFTER_COMMA)} miles")
    else:
        print("No cars left.")
...
def car_fleet_creating():
    for _ in range(configfile.CARS_IN_FLEET):
        car_fleet.append(Car(mileage=configfile.DEFAULT_MILEAGE, fuel=configfile.DEFAULT_FUEL,
                         economy=random.uniform(configfile.MIN_ECONOMY, configfile.MAX_ECONOMY),
                         color=generate_random_color(), model=random.choice(configfile.models)))
